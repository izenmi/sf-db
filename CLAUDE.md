...
- **`seriesName` はエンティティ化しない**。ソースは works.json の自由文のままで、
  ...
  `/series`(一覧)と `/series/<シリーズ名>`(詳細)のページと絞り込みを賄う。
  ...
  - **表記ゆれは統合してから登録する**。「◯◯」と「◯◯シリーズ」が並ぶと別シリーズになる。
    2026-08-12に37件を統合した(敵は海賊/星界/巨人たちの星/マルドゥック/デューン/移動都市クロニクル/
    星のパイロット/司政官/博物館惑星/ダーティペア/戦闘妖精・雪風/人類補完機構/スプロール三部作/
    ジーリー・クロニクル/老人と宇宙/航空宇宙軍史/図書館戦争/知性化/神シリーズ)
  - **同名別シリーズは著者名で分ける**。『火星三部作』がキム・スタンリー・ロビンスンと神林長平で
    衝突していたので、後者を『火星三部作(神林長平)』にした
...
| `byauthor.py [著者id…]` | **カタログ由来の候補源**。楽天ブックスの`author=`検索で、**登録済み著者の未登録作**を列挙する。書誌と紹介文が1回のリクエストで揃うのでWikipedia記事のない作品も拾える。引数なしで works.json の著者を刊行数順に全員たどる |
...
→ **592作品**(同日、Wikipediaカテゴリを候補源に切り替え、バッチ51〜52)
→ **816作品**(同日、楽天の著者検索を候補源に切り替え、バッチ53〜74)
→ **964作品**(同日、WikipediaのSF小説カテゴリに戻す、バッチ75〜93。**この候補源はここで打ち止め**)。
...
| WikipediaのSF小説カテゴリ | 169作品 | 51〜52, 75〜93 |
| 楽天ブックスの著者検索(`byauthor.py`) | 224作品 | 53〜74 |

964作品(国内499/海外465)・著者357・翻訳者160・出版社69・テーマ37・アワード12。
受賞歴は748件。発表年は1946年『火星からの来訪者』から2026年まで。

## 候補源としての楽天・NDL(2026-08-12に実測)

- **楽天のジャンル facet は使えない**。`BooksGenre/Search` で確認したところ SF の葉ジャンルは
  `001004002`(SF・ホラー)ただ一つで在庫195件しかなく、文庫側(`001019001`)には下位ジャンルが無い。
  `harvest.py` が伸びない理由はこれ
- **NDLの件名 facet も使えない**。opensearch の `subject` パラメータは無視される(全件が返る)。
  SRU(`/api/sru`、CQL)なら `subject="科学小説"` が効くが309件しかなく、大半が研究書・全集。
  NDLは小説本体に件名を付けないため
- **効くのは「著者引き」**。楽天 `author=` は正確で、ISBN・版元・刊行日・紹介文が1回で揃う
  (`byauthor.py`)。NDL opensearch の `creator=` は絶版書まで拾えるので刊行年の裏取りに使う。
  登録済み289著者を舐めて未登録候補1622件、うちSF系版元・紹介文ありに絞って417件が得られた

## 未着手の候補源(次に手をつけるところ)

- **楽天の著者検索の残り**(`python3 scripts/byauthor.py`)。2026-08-12時点で、SF系4版元(早川/創元/
  国書/竹書房)の分と、他版元の主要SF作家(小林泰三・星新一・機本伸司・新井素子・眉村卓・瀬名秀明・
  貴志祐介・恩田陸ほか)の分まで消化した。**残るのは非SF比率の高い層**で、紹介文が110字以上あるものだけで
  約400件あるが、その大半はミステリ・時代小説・エッセイ・シリーズ巻違い・アンソロジー。
  1件ずつ判断が要り、採用率は1〜2割程度と見込まれる

- **WikipediaのSF小説カテゴリ**(`python3 scripts/wikicat.py`)。手順は確立した。
  (1) `wikicat.py` で未登録の記事名を出す(2026-08-12時点で478件) →
  (2) `wikiinfo.py --lead-only` で導入部を一括取得(20件/リクエスト、444件が取得できた) →
  (3) 導入部に**登録済み著者の名前が含まれる行**だけ残す(153件) →
  (4) 導入部の「短編SF小説」「シリーズ作品の総称」「アニメーション映画」等を弾いて書籍だけにする →
  (5) `lookup.py` に流して DUP 判定と紹介文取得。**バッチ75〜79でこの層から54件を追加した**
- **この候補源は2026-08-12に打ち止めまで回した**(バッチ75〜93、169作品)。既登録著者分から始めて、
  未登録著者分に眠っていたSF古典(『1984年』『すばらしい新世界』『タイム・マシン』『宇宙戦争』
  『海底二万里』『フランケンシュタイン』『華氏451度』『火星年代記』『渚にて』『わたしを離さないで』
  『火星のプリンセス』『宇宙のスカイラーク』『家畜人ヤプー』ほか)まで拾い切った。
  賞パイプラインで作ったDBには賞の創設より古い作品が構造的に入らない、というのがこの層の正体だった
- **残り107件は登録できない**。内訳は (1) シリーズ全体の記事(レンズマン、キャプテン・フューチャー、
  ペリー・ローダン、火星シリーズ等)、(2) ゲーム・アニメ・特撮のノベライズ、(3) 短編単独の記事、
  (4) 絶版で紹介文もあらすじ節も存在しない作品、(5) 別邦題での重複。**すべて pending.txt に理由付きで
  記録した**ので、再挑戦するときはそこを見れば同じ調査を繰り返さずに済む
- 打ち止め間際の3バッチ(91〜93)は候補29件に対して採用7件まで落ちた。次に伸ばすなら別の候補源を探すこと
...
- **この層は絶版が多く、紹介文と書影の両方が取れないことがある**。`lookup.py` で紹介文が空だったら
  `wikiinfo.py`(あらすじ節)→`plot.py`(英語版)→`kobo.py` の順に当たり、それでも無ければ pending.txt 行き。
  書影も同様で、`setcover.py --search <題> --kobo` で電子版だけ残っていることがある(『天空の劫火』等)
...
- **同点が支配的になる**(選んだテーマだけを持つ作品は全部100%)。id昇順だけだと
  スラッグ順という無意味な並びになるので、メディアミックス数→受賞件数→id昇順で崩している
- **データは `generated/recommend-index.json`**(テーマ表 + 各作品のテーマidだけ)。
  これだけで選択UIが動き、**works.json はテーマを1つ選んでから**取りに行く。
  **この索引の読み手は /recommend だけ。ページを消すなら生成も消すこと**。spoilerテーマ3件は選択肢からも集計からも外している
...
- **ネタバレテーマは `visibleThemes()` で必ず外す**(df・norm・dot・ラベルのすべて)。ビルド側
  ...
  「一致したテーマ」のラベルからネタバレが漏れる。**このサイトで最も壊しやすい箇所**
- **スコアは各シードとの類似度(ビルド側 relatedIdsFor と同一式: テーマIDFコサイン + 同著者+0.15)を
  個別に計算して算術平均**。シード1件のとき詳細ページの関連6件と同じ順位になる(同点の並びだけは、
  こちらが映像化→受賞のタイブレークを足すぶん異なる。ビルド側はid昇順のみ)。
  ボーナスで1.0を超えるので **%表示は `Math.min(100, …)` で頭打ち**
